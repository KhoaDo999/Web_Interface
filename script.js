...
let isFirstLoad = true; // Thêm biến để kiểm tra lần đầu load
...
function setupRealtimeAlert() {
    ...
    historyRef.on("child_added", (snapshot) => {
        ...
        if (message.toLowerCase().includes("buồn ngủ") && time) {
            // KHÔNG hiện popup khi mới load trang (chỉ hiện cho bản ghi mới)
    ...
    // Sau khi load xong dữ liệu cũ, đánh dấu đã load xong
    historyRef.once("value").then(() => {
        isFirstLoad = false;
        console.log("✅ Đã load xong dữ liệu cũ, sẵn sàng nhận cảnh báo mới");
    });
...
function showAlertPopup(time) {
    console.log("Hiển thị popup với thời gian:", time); // Thêm dòng này

    const popup = document.getElementById('alertPopup');
    const timeElement = document.getElementById('alertTime');

    if (!popup) {
        console.error("Không tìm thấy popup trong HTML!");
        return;
    }
